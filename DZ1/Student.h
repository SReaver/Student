...
public:
	Student() { this->sID = 0; this->name = ""; this->surname = ""; this->otch = ""; this->birthday=Date(); this->address = ""; this->tel = ""; this->kurs = 0; this->fak=Faculty(); }
	...
	friend ostream & operator << (ostream& out, Student s); //перегрузка оператора вывода для класса contact
	friend istream & operator >> (istream& in, Student &s);
};
ostream & operator << (ostream& out, Student s) //перегрузка оператора вывода для класса contact
{
	out << left<<s.sID << " "<<setw(15) << s.name << " " <<setw(20)<< s.surname << " " << s.otch<<" "<<s.birthday<<" "<<s.address<<" "<<s.tel<<" "<<s.kurs<<" "<<s.fak << endl;
	return out;
}

istream & operator >> (istream& in, Student &s)
{
	in >> s.sID >> s.name >> s.surname >> s.otch >> s.birthday;
	getline(in, s.address, '&');
	in >> s.tel >> s.kurs >> s.fak;
	return in;
}
